    ...
    def fit(self, X, y):
        ...
        for i in range(self.__epochs):
            ...
            # 1- Calculate the net input W^T * x
             res = self.__net_input(X)
            # 2- Get the activation using Sigmoid function
             hyp =  self.__activation(res)
            # 3- Calculate the gradient
             err = (y - hyp)
             tmp = X.T.dot(err)
            # 4- Update the weights and bias using the gradient and learning rate
             self.w_[0] = self.w_[0] + self.__learning_rate*err.sum()
             self.w_[1:] = self.w_[1:] + self.__learning_rate * tmp
            # 5- Uncomment the cost collecting line
             self.cost_.append(self.__logit_cost(y, self.__activation(res)))
